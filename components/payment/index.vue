<template>
  <view class="payment-container">
    ...
    <pay-result v-if="isShowPayResult" @closeDialog="closeDialog" @closeHandle="closeHandle" @goNextHandle="goNextHandle" @payAgainHandle="payAgainHandle" @goBack="goBack" :payReturnMsg="payReturnMsg" :btnText="btnText" :buyType="buyType" :appointmentTime="appointmentTime">
    </pay-result>
...
<script>
  ...
  // #endif
  import { navigateTo, pollIntervalManager, getQueryString, debounce } from '@/utils/util'
  ...
  export default {
    ...
    props: {
      ...
      buyType: {
        type: String,
        default: '1'  // 1=>智能评估购买 5=>训练购买  2=>人工评估购买(需要填写表单预约时间) 3=>服务咨询购买(需要填写表单预约时间) 4=>趣味锻炼 6 =>健脑运动
      },
    ...
    methods: {
      preventTouchMove() {},
      payCallback() {
        console.log('asdf23423', this.buyType)
        if (!this.isPad) {
      ...
      payPreHandle: debounce(function() {
      ...
      createOrder() {
        ...
        if ((this.buyType == 4 || this.buyType == 6) && !this.agreementChecked) {
          // 服务包购买
          return uni.showToast({
            title: '请勾选服务协议',
            icon: 'none',
        ...
        this.$request({
          ...
          if (res.status === 1000) {
            ...
            if (res.data.orderType == 2 && this.buyType != 1) {
              // 支付成功(免单转态，不走支付流程)
            ...
            if (this.buyType == 1) {
              // 认知障碍筛查（智能评估），不支付，先创建订单跳转信息填写进行筛查，在支付解锁报告
              navigateTo('/pages/evaluation/sort?payOrderId='+res.data.payOrderId)
              // getApp().goEvalution({
              //   token: this.userInfo.token,
              //   requestType: 1,
              ...
              // 其他类型商品，创建订单之后，跳转至支付选择页进行支付
              navigateTo('/pages/pre-pay/index?payOrderId='+res.data.payOrderId+'&orderType='+res.data.orderType)
